...
{
while(1){
    //遍历buffer 找到PB_START 找到之后 解析出整包长度 然后得到结束符位置判断是否为PB_END 
...
if(parse_success){
    ...
    int method_name_len_index = req_id_index+message->m_req_Id_len;
     if(method_name_len_index>=end_index){
        message->parse_success = false;
        ERRORLOG("parse error,method_name_index [%d] > end_index [%d]",method_name_len_index,end_index);
        continue;
    }
    ...
    message->m_err_code = getInt32FromNetByte(&tmp[err_code_index]);
     ...
     if(err_info_len_index>=end_index){
        message->parse_success = false;
        ERRORLOG("parse error,method_name_index [%d] > end_index [%d]",err_info_len_index,end_index);
        continue;
    }
